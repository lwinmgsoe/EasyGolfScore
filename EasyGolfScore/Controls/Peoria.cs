...
{
    public partial class Peoria : UserControl
    {
        private static Peoria _instance;
        public static Peoria Instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new Peoria();
                }
                return _instance;
            }
        }
        ...
        private void standardHoleAndStrokes_DoubleClick(object sender, EventArgs e)
        {
        ...
        {
            var marks = standardHoleAndStrokes.GetSelectedCells();
            foreach (var mark in marks)
            {
                MessageBox.Show($"{mark.HoleNo} - {mark.Stroke}");
            }
        ...
        private void Peoria_Load(object sender, EventArgs e)
        {
            if (!DesignMode)
            {
        ...
        private double CalculateHandicapByPeoria(List<MarkRule> selectedHoles, int row = 0, int totalPar = 72, int holeNo = 18)
        {
            int total = 0;
            for(int i = 0; i < holeNo; i++)
            {
                ...
                var hole = selectedHoles.Where(x => x.HoleNo == (col-1).ToString()).FirstOrDefault();
                if(hole != null)
                {
                    var _stroke = int.Parse(dgStroke[col, row].Value.ToString());
                    //MessageBox.Show($"index-{i} col- {col} hole {hole.HoleNo} user stroke - {_stroke}");
                    total = total + _stroke;
                }
                else
                {
            ...
            var multiply = total * 1.5;
            var substractPar = multiply - totalPar;

            var handi = Math.Round(substractPar * 0.8, 3);
            return handi;
        }
        private void calculateMark()
        {
            ...
            for (int j = 0; j < rowcount; j++)
            {
                ...
                //calculate handicap by new peoria
                var selectedHoles = standardHoleAndStrokes.GetSelectedCells();
                if(selectedHoles == null || selectedHoles.Count < 12)
                {
                    MessageBox.Show("Please select holes", "selece 12 holes", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                var Handi = CalculateHandicapByPeoria(selectedHoles, j, stdStroke, numOfHoles);
                var Net = sTotal - Handi;
                dgStroke[colcount - 4, j].Value = um.TotalMark;
                dgStroke[colcount - 3, j].Value = Handi;
                dgStroke[colcount - 2, j].Value = Net;
                var diff = Math.Round(Net - stdStroke);
                var strDiff = "0";
                if (diff > 0)
                {
        ...
        private void calculateMarkSort()
        {
            ...
            for (int i = 0; i < row; i++)
            {
                ...
                sm.TotalMark = int.Parse(dgStroke[column - 4, i].Value.ToString());
                sm.HS = double.Parse(dgStroke[column - 3, i].Value.ToString());
                sm.NetMark = double.Parse(dgStroke[column - 2, i].Value.ToString());
            ...
            for (int i = 0; i < row; i++)
            {
                ...
                dgStroke[column - 3, i].Value = lstSortMarks[i].HS;
                dgStroke[column - 4, i].Value = lstSortMarks[i].TotalMark;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            using (var pk = new ExcelPackage())
            {
                ...
                sheet.Cells[1, 1].Value = "Golf Marks by System New Peoria";
                ...
                sheet.Cells[2, holes + 4].Value = "Total Marks";
                sheet.Cells[2, holes + 5].Value = "H/S";
        ...
        private void dgStroke_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
